...
def split_data(data_dir,augment_both=True):
    ...
    tmp_img = cv.imread(os.path.join(
        os.path.join(img_dir, 'real'), x_list[0]), cv.IMREAD_GRAYSCALE)
    ...
    for i in range(n):
        x_tmp = cv.imread(os.path.join(
            os.path.join(img_dir, 'real'), x_list[i]), cv.IMREAD_GRAYSCALE)
        ...
        if augment_both:
            gt_tmp = cv.imread(os.path.join(
                os.path.join(gt_dir, 'real'), gt_list[i]), cv.IMREAD_GRAYSCALE)
...
def gamma_data(data_dir,augment_both=True, in_gt=False):
    ...
    for i in (dir_list):
        ...
        if augment_both:
            ...
            print("Working on the dir: ", os.path.join(X_dir, i))
        for j in range(n):
            x_tmp = cv.imread(os.path.join(X_dir, os.path.join(i, X_list[j])), cv.IMREAD_GRAYSCALE)
            if not in_gt:
            # Normalization and gamma correction for grayscale image
                x_tmp = np.float32(x_tmp) / 255  # Normalize to 0-1
                gam30_x = np.uint8(np.power(x_tmp, 0.3030) * 255)
                gam60_x = np.uint8(np.power(x_tmp, 0.6060) * 255)
            else:
                gam30_x = x_tmp
                gam60_x = x_tmp
            ...
            if augment_both:
                gt_tmp = cv.imread(os.path.join(GT_dir, os.path.join(i, GT_list[j])), cv.IMREAD_GRAYSCALE)
            cv.imwrite(os.path.join(save_dir_x30, X_list[j]), gam30_x)
            cv.imwrite(os.path.join(save_dir_x60, X_list[j]), gam60_x)
